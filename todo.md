...
- Instructions:
  - Start all 4 services: MongoDB, Flask AI (`python app.py`), Hardhat node (`npx hardhat node`), Express server (`npm run dev`), React client (`npm run dev`)
  - Register a user account → login → go to Analyze → fill form → check AI result
  - Submit a report → check it appears in My Reports
  - Login as agency → see the report in All Reports
  - Open the report → change status to "escalated" → confirm blockchain hash appears
  - Go to Blockchain Audit page → confirm the record appears

---

**Task 9.2 — Error Handling**
- Instructions:
  - In Express: add a global error handler in `server/middleware/errorMiddleware.js` → catch all unhandled errors → return `{ message: error.message }` with appropriate status code
  - In React: wrap API calls in try/catch → show a toast or error message div on failure
  - In Flask: wrap predict logic in try/except → return `{"error": "..."}` with status 500

---

**Task 9.3 — Seed Agency Account**
- Instructions:
  - In `server/` create `seed.js`:
    - Directly insert a User document with `role: 'agency'`, fixed email/password
    - Run once: `node seed.js`
  - This ensures you always have an agency login ready for demo without manual setup
...
```
Phase 1  →  Setup all 4 projects
Phase 2  →  Backend auth (login/register working)
Phase 3  →  AI model trained and Flask running
Phase 4  →  Reports + Analysis API done
Phase 5  →  Smart contract deployed + backend logs to it
Phase 6  →  React routing + login/register UI
Phase 7  →  User dashboard complete
Phase 8  →  Agency dashboard complete
Phase 9  →  Test full flow end to end
```

---

Want me to start writing the actual code for any specific phase?